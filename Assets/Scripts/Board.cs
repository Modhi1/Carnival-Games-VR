...
using UnityEngine.UI;
    ...
    private void Awake()
        ...
        private void Start()
        {
            ...
            no_of_rows = symbolsArray.GetLength(0);
            no_of_columns = symbolsArray.GetLength(1);

            SetNumOfAvailableCells();
        ...
        // loop over the symbols array and set it to default value
        SetSymbolsArray();

        // reset number of avaliable cells
        SetNumOfAvailableCells();

        // loop over all the buttons and change the image
        foreach (Button button in buttonsArray)
        {
              Image buttonImage = button.GetComponent<Image>();

              buttonImage.sprite = spriteBlock;
        }

        // make it true so that always player X play first 
        player1 = true;


    }

        private void SetNumOfAvailableCells()
    {
